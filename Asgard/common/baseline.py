"""Baseline Management -- creating, loading, and filtering violations against baselines."""
...
class BaselineManager:
    ...
    def save(self) -> None:
        ...
        data = {
            ...
            'entries': [
                {
                    'item_id': e.item_id, 'item_type': e.item_type,
                    'location': e.location, 'message': e.message,
                    'reason': e.reason, 'created_at': e.created_at,
                    'created_by': e.created_by, 'expires_at': e.expires_at,
                    'metadata': e.metadata,
                }
    ...
    def create_from_items(
        self, items: List[T], item_type: str,
        location_func: Callable[[T], str],
        message_func: Optional[Callable[[T], str]] = None,
        reason: str = "Initial baseline", created_by: str = "asguardian",
        expiry_days: Optional[int] = None,
    ) -> int:
        """Create baseline entries from a list of items. Returns count created."""
        baseline = self.load()
        count = 0
        ...
        for item in items:
            ...
            if baseline.find_match(location, item_type, message, self.config.fuzzy_matching):
                continue
            entry = BaselineEntry(
                item_id=item_id, item_type=item_type, location=location,
                message=message, reason=reason, created_by=created_by,
    ...
    def filter_items(
        ...
        """Filter items against the baseline. Returns items NOT in baseline."""
        baseline = self.load()
        new_items = []
        for item in items:
            location = location_func(item)
            message = message_func(item) if message_func else ""
            match = baseline.find_match(location, item_type, message, self.config.fuzzy_matching)
            if match is None:
                new_items.append(item)
        return new_items

    def add_entry(
        self, location: str, item_type: str, message: str = "",
        reason: str = "", created_by: str = "asguardian",
        expiry_days: Optional[int] = None,
    ) -> bool:
        """Manually add a baseline entry. Returns True if added."""
        baseline = self.load()
        if baseline.find_match(location, item_type, message, self.config.fuzzy_matching):
            return False
        expiry_days = expiry_days or self.config.default_expiry_days
        ...
        entry = BaselineEntry(
            item_id=self._generate_id(location, item_type, message),
            item_type=item_type, location=location, message=message,
...
class BaselineMixin:
    """Mixin to add baseline support to any scanner/analyzer."""
