...
async def check_skip_link(page: Page) -> bool:
    ...
    skip_selectors = [
        ...
        'a[href="#maincontent"]', 'a[href="#skip"]', '.skip-link',
        '.skip-to-content', '[class*="skip"]', 'a:first-child[href^="#"]',
    ]
        ...
        try:
            ...
            style_js = """
                (element) => {
                    ...
                    return {
                        outline: computed.outline, outlineColor: computed.outlineColor,
                        outlineWidth: computed.outlineWidth, outlineStyle: computed.outlineStyle,
                        boxShadow: computed.boxShadow, border: computed.border,
                        backgroundColor: computed.backgroundColor,
                    };
                }
            """
            styles_before = await page.evaluate(style_js, element)
            await element.focus()
            styles_after = await page.evaluate(style_js, element)
            has_visible_focus = (
