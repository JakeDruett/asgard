            ...
            () => {
                ...
                for (const el of elements) {
                    ...
                    if (el.scrollWidth > el.clientWidth &&
                        ...
                        results.push({
                            selector: selector, type: 'horizontal',
                    ...
                    if (rect.right > window.innerWidth || rect.left < 0) {
                        const selector = el.id ? '#' + el.id :
                            ...
                            el.tagName.toLowerCase();
                        results.push({
                            selector: selector, type: 'viewport',
        ...
        for overflow in overflows:
            ...
            issues.append(LayoutIssue(
                ...
                affected_area=ElementBox(
                    ...
                    width=overflow["width"], height=overflow["height"],
                    selector=selector,
                ),
            ...
            () => {
                ...
                for (let i = 0; i < interactive.length; i++) {
                    for (let j = i + 1; j < interactive.length; j++) {
                        ...
                        if (overlap) {
                            const sel1 = interactive[i].id ? '#' + interactive[i].id : interactive[i].tagName.toLowerCase();
                            const sel2 = interactive[j].id ? '#' + interactive[j].id : interactive[j].tagName.toLowerCase();
                            results.push({
                                selector1: sel1, selector2: sel2,
                ...
                for (const container of containers) {
                    ...
                    if (uniqueTops.length > 1) {
                        ...
                        if (variance > 5 && variance < 50) {
                            ...
                            results.push({ selector: selector, variance: variance });
                        }
                    }
                }
                return results.slice(0, 10);
            ...
            () => {
                ...
                for (const el of textElements) {
                    ...
                    if (!isNaN(lineHeight) && !isNaN(fontSize) && lineHeight < fontSize * 1.2) {
                        ...
                        results.push({ selector: selector, type: 'line-height', lineHeight: lineHeight, fontSize: fontSize });
                    }
                }
                return results.slice(0, 10);
            }
                ...
                for (const el of interactive) {
                    ...
                    if (rect.width < 24 || rect.height < 24) {
                        if (rect.width > 0 && rect.height > 0) {
                            const selector = el.id ? '#' + el.id : el.tagName.toLowerCase();
                            results.push({ selector: selector, type: 'too_small', width: rect.width, height: rect.height });
                        }
                    }
                    if (style.opacity === '0') {
                        const selector = el.id ? '#' + el.id : el.tagName.toLowerCase();
                        results.push({ selector: selector, type: 'invisible' });
                    }
                }
                return results.slice(0, 10);
            }
