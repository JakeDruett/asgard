...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="heimdall",
        description="Heimdall - Code Quality Control",
...
def _setup_deps_commands(subparsers) -> None:
    ...
    # Export subcommand with its own args (dot, json, mermaid)
    export_parser = deps_subparsers.add_parser("export", help="Export dependency graph (dot, json, mermaid)")
...
COMMAND_KNOWN_SUBCOMMANDS = {
    "security": {"scan", "secrets", "dependencies", "vulnerabilities", "crypto", "access", "auth", "headers", "tls", "container", "infra"},
...
def main(args=None):
    ...
    if args.command == "quality":
        if not hasattr(args, 'quality_command') or args.quality_command is None:
            ...
            print("  datetime, typing, thread-safety, race-conditions, daemon-threads,")
            print("  future-leaks, blocking-async")
    ...
    # Handle dependencies subcommands
        ...
        if not hasattr(args, 'deps_command') or args.deps_command is None:
            print("Error: Please specify a dependencies command (e.g., 'analyze', 'cycles', 'export')")
            sys.exit(1)
